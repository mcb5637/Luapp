...
#pragma warning(disable : 4702)
	...
	class LuaSerializer {
		...
		// a reference to an already serialized/deserialized upvalue
		...
		LType DeserializeType() {
			...
			if ((static_cast<int>(r) < static_cast<int>(LType::Nil) || static_cast<int>(r) > static_cast<int>(LType::Thread)) && r != ReferenceType &&
				r != UpvalueReferenceType && r != IntegerType && r != GlobalsRefType)
		...
		void PrepareDeserialize() {
			...
			if (v == FileVersion) {
				...
				throw std::format_error{"invalid file version"};
			}
			L.NewTable();
			IndexOfReferenceHolder = L.GetTop();
		}
			...
			if constexpr (State::Capabilities::UpvalueId) {
				...
				for (int n = 1; n <= i.NumUpvalues; ++n) {
					...
					if (upRef == UpRefs.end()) {
						...
						WritePrimitive(upRef->second.FuncReference);
						WritePrimitive(upRef->second.UpvalueNum);
					}
				}
			}
		}
		...
		if constexpr (State::Capabilities::UpvalueId) {
			...
			for (int n = 1; n <= n_upvalues; ++n) {
				...
				if (t == UpvalueReferenceType) {
					int funcRref = ReadPrimitive<int>();
					int upNum = ReadPrimitive<int>();
					L.GetTableRaw(IndexOfReferenceHolder, funcRref);
					if (L.IsNil(-1))
