...
namespace LuappDev {
	...
	namespace cls {
		...
		public:
			...
			InheritanceTestB operator+(const InheritanceTestB& o) const {
				return InheritanceTestB{i + o.i};
			}
			InheritanceTestB operator-() const {
				return InheritanceTestB{-i};
			}
		};
		template<class S>
		...
		public:
			...
			InheritanceTestD operator+(const InheritanceTestD& o) const {
				return {InheritanceTestB<S>::i + o.i, j + o.j};
			}
		};
		template<class S>
		...
		public:
			static constexpr int NumberUserValues = S::Capabilities::UserValues ? (S::Capabilities::ArbitraryUserValues ? 3 : 1) : 0;
	...
	TEST_CASE_TEMPLATE("UserClass", US, lua::UniqueState, ExtUniqueState) {
		...
		{
			US L2{};
			L2.template NewUserClass<cls::DtorTest>([&closed]() {
				closed = true;
			});
		...
		{
			US L2{};
			L2.template NewUserClass<InheritanceTestF>([&closed]() {
				closed = true;
			});
		...
		{
			US L2{};
			L2.template NewUserClass<MultiInheritance>([&num_closed]() {
				++num_closed;
			});
