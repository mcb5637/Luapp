...
namespace LuappDev {
	...
	TEST_CASE_TEMPLATE("Functions", US, lua::UniqueState, ExtUniqueState) {
		...
		std::array toregobj{
			S::FuncReference::template GetRef<&A::G>(a, "G"),			S::FuncReference::template GetRef<&A::GC>(a, "GC"),
			S::FuncReference::template GetRef<&A::API>(a, "API"),		S::FuncReference::template GetRef<&A::APIC>(a, "APIC"),
		...
		L.SetTop(0);
		L.Push(42);
		L.PushLambda([i = 0](S l) mutable {
			l.Push(i++);
			l.PushValue(S::UpvalueIndex(2));
			return 2;
		}, 1);
		L.PushValue(1);
		...
		L.PushLambda([i = 0](int x) mutable {
			return i += x;
		});
		...
		L.PushLambda([](std::optional<int> x) {
			return x;
		});
		CHECK(L.Debug_GetUpvalue(1, 1) == nullptr);
		L.PushValue(1);
