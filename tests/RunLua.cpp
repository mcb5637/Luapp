...
namespace LuappDev {
	...
	TEST_CASE_TEMPLATE("RunLua", US, lua::UniqueState, ExtUniqueState) {
		...
		if (L.Version() == 5.0)
			CHECK_EQ(std::string{"<function Lua global fool (defined in: [string \"function fool(a, b) return a+b end\"]:1)>"}, L.ToDebugString(1));
		else
			CHECK_EQ(std::string{"<function Lua  fool (defined in: [string \"function fool(a, b) return a+b end\"]:1)>"}, L.ToDebugString(1));
