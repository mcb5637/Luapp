...
namespace lua::v50 {
	...
	static_assert(DebugInfo::ShortSrcSize == LUA_IDSIZE);
	...
	void CopyDebugInfo(const lua_Debug& src, DebugInfo& trg) {
		...
		std::memcpy(trg.ShortSrc, src.short_src, DebugInfo::ShortSrcSize);
		trg.ShortSrc[DebugInfo::ShortSrcSize - 1] = '\0';
	...
	void State::Copy(int source, int target) {
		target = ToAbsoluteIndex(target);
		PushValue(source);
		Replace(target);
	}
	...
	void State::Push(CFunction f, int numUpvalues) {
		lua_pushcclosure(L, f, numUpvalues);
	}
	...
	int State::Arithmetic_Unprotected(lua_State* L) {
		...
		switch (op) {
			case ArithmeticOperator::Add:
				lua_dostring(L, "return function(a, b) return a + b; end");
				break;
			...
			case ArithmeticOperator::Modulo:
				lua_dostring(L, "return function(a, b) return math.mod(a, b); end");
				break;
			case ArithmeticOperator::Pow:
				lua_dostring(L, "return function(a, b) return a ^ b; end");
				break;
			case ArithmeticOperator::UnaryNegation:
				lua_dostring(L, "return function(a) return -a; end");
				break;
			default:
				lua_pushnil(L);
		}
	...
	int State::Next_Unprotected(lua_State* L) {
	...
	const char* Debug_GetOptionString(DebugInfoOptions opt, bool pushFunc, bool fromStack) {
		...
		static constexpr auto lut =
			lut_details::MakeLUT<int,
								 static_cast<int>(DebugInfoOptions::Name | DebugInfoOptions::Source | DebugInfoOptions::Line | DebugInfoOptions::Upvalues | push | stack),
								 lut_details::OptionStringElement{static_cast<int>(stack), '>'}, lut_details::OptionStringElement{static_cast<int>(push), 'f'},
		...
		opt = opt & (DebugInfoOptions::Name | DebugInfoOptions::Source | DebugInfoOptions::Line | DebugInfoOptions::Upvalues);
		if (pushFunc)
			opt = opt | push;
		if (fromStack)
			opt = opt | stack;
	...
	const char* State::Debug_GetLocal(int lvl, int localNum) {
		lua_Debug ar;
		if (!lua_getstack(L, lvl, &ar))
			return nullptr;
		return lua_getlocal(L, &ar, localNum);
	}
	const char* State::Debug_SetLocal(int lvl, int localNum) {
		lua_Debug ar;
		if (!lua_getstack(L, lvl, &ar))
			return nullptr;
	...
	ErrorCode State::DoFile(const char* filename) {
		return static_cast<ErrorCode>(luaL_loadfile(L, filename) || lua_pcall(L, 0, LUA_MULTRET, 0));
	}
